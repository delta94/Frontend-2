...
export function createNodeModel(code: string, id: string, label?: string): FlowNodeModel | null {
  ...
  if (code === 'SOURCE' || code === StartNodeModel.TYPE) return new StartNodeModel(uuid, label);
  if (code === 'PROCESS' || code === ProcessNodeModel.TYPE) return new ProcessNodeModel(uuid, label);
  if (code === 'GATEWAY' || code === DecisionNodeModel.TYPE) return new DecisionNodeModel(uuid, label);
...
function getNodeCode(type: string) {
  switch (type) {
    case StartNodeModel.TYPE:
      return 'SOURCE';
    case ProcessNodeModel.TYPE:
      return 'PROCESS';
    case DecisionNodeModel.TYPE:
      return 'GATEWAY';
